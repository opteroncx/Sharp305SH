...
#define CONFIG_SHSYS_CUST_PERFLOCK 1   //------perflock

#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-089 */
#include <mach/perflock.h>
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 13-089 */

/* SH_AUDIO_DRIVER-> */ /*15-061*/
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-006 */
#define TAPAN_DEBUGMODE_MSK     0x01
#define TAPAN_DEBUGMODE_ON      0x01
#define TAPAN_DEBUGMODE_OFF     0x00
#define TAPAN_MICBIAS_MSK       0x02
#define TAPAN_MICBIAS_ON        0x02
#define TAPAN_MICBIAS_OFF       0x00
#define TAPAN_CODECSTOP_MSK     0x04
#define TAPAN_CODECSTOP_ON      0x04
#define TAPAN_CODECSTOP_OFF     0x00
#endif /* CONFIG_SH_AUDIO_DRIVER *//* 13-006 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-006 */
struct tapan_priv *tapan_switch;
static int bias_mode_for_testmode = 0;
static bool irq_mode_for_testmode  = true;

int tapan_codec_get_bias_mode(void)
{
	return bias_mode_for_testmode;
}
EXPORT_SYMBOL_GPL(tapan_codec_get_bias_mode);

void tapan_codec_set_bias_mode(int mode)
{
	struct wcd9xxx_mbhc mbhc = tapan_switch->mbhc;
	bias_mode_for_testmode = mode;

	if(irq_mode_for_testmode){
		if((bias_mode_for_testmode & TAPAN_CODECSTOP_MSK) == TAPAN_CODECSTOP_ON ){
		 ...
		 if((bias_mode_for_testmode & TAPAN_CODECSTOP_MSK) == TAPAN_CODECSTOP_OFF){
...
EXPORT_SYMBOL_GPL(tapan_codec_set_bias_mode);
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-083 */
static struct perf_lock hkmode_a2dp_perf_lock;
static int hkmode_a2dp_flg = 0;

void tapan_codec_set_hkmode_a2dp(int mode){
	if(mode){
		if(hkmode_a2dp_flg == 0){
			perf_lock(&hkmode_a2dp_perf_lock);
			hkmode_a2dp_flg = 1;
			pr_debug("%s   hkmode_a2dp is set \n",__func__);
		}
	}else{
		if(hkmode_a2dp_flg != 0){
			perf_unlock(&hkmode_a2dp_perf_lock);
			hkmode_a2dp_flg = 0;
			pr_debug("%s   hkmode_a2dp is unset \n",__func__);
		}
	}
}
EXPORT_SYMBOL_GPL(tapan_codec_set_hkmode_a2dp);
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 13-083 */

#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-089 */
static struct perf_lock playback_status_perf_lock;
static int playback_status_flag = 0;
static int playback_clock_status_flag = 0;
struct mutex playback_lock;

void tapan_codec_set_playback_status(int mode, int from) {
	mutex_lock(&playback_lock);
	switch (from) {
	case 1:
		pr_debug("%s: Request from A2DP playback\n", __func__);
		if (mode)
			playback_status_flag |= 0x01;
		else
			playback_status_flag &= (~0x01);
		break;
	case 2:
		pr_debug("%s: Request from Regualer playback\n", __func__);
		if (mode)
			playback_status_flag |= 0x10;
		else
			playback_status_flag &= (~0x10);
		break;
	default:
		pr_err("%s: Unsupported playback\n", __func__);
	}

	if ((playback_status_flag != 0) && (playback_clock_status_flag == 0)) {
		perf_lock(&playback_status_perf_lock);
		playback_clock_status_flag = 1;
		pr_debug("%s   playback_status is set \n",__func__);
	} else if ((playback_status_flag == 0) && (playback_clock_status_flag != 0)) {
		perf_unlock(&playback_status_perf_lock);
		playback_clock_status_flag = 0;
		pr_debug("%s   playback_status is unset \n",__func__);
	}
	mutex_unlock(&playback_lock);
}
EXPORT_SYMBOL_GPL(tapan_codec_set_playback_status);
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 13-089 */
	...
	struct snd_kcontrol *kcontrol, int event)
{
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-040 */
	struct wcd9xxx_mbhc mbhc = tapan->mbhc;
#endif /* CONFIG_SH_AUDIO_DRIVER *//* 13-040 */
...
else /* CONFIG_SH_AUDIO_DRIVER */ /* 13-006 */
			if (++tapan->micb_2_users == 1)
				wcd9xxx_resmgr_add_cond_update_bits(
						&tapan->resmgr,
						WCD9XXX_COND_HPH_MIC,
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		if (micb_ctl_reg == WCD9XXX_A_MICB_1_CTL) {
			u8 mbhc_micb_ctl_val = snd_soc_read(codec, TAPAN_A_MICB_2_CTL);
			if (mbhc_micb_ctl_val & 0x80) {
				snd_soc_update_bits(codec, TAPAN_A_MICB_2_CTL, 0x80, 0x00);
			}
		}
#endif /* CONFIG_SH_AUDIO_DRIVER *//* 13-040 */
				 ...
				 struct snd_soc_dai *dai)
{
	...
	if (rc < 0)
		pr_err("%s: tapan_hw_params error %d\n", __func__, rc);
				       ...
				       unsigned int *rx_slot)
{
	...
	if (remote_route_enable(dai)) {
		...
		if (rc < 0)
			pr_err("%s: tapan_get_channel_map error %d\n",
			       __func__, rc);

		rc = tapan_set_channel_map(dai, tx_num,
					tapan_tx_slot, rx_num, tapan_rx_slot);
		if (rc < 0)
			pr_err("%s: tapan_set_channel_map error %d\n",
			       __func__, rc);
...

{
	...
	if (remote_route_enable(dai))
		...
		rc = tapan_get_channel_map(dai, tx_num, tx_slot,
					rx_num, rx_slot);
...
/* SH_AUDIO_DRIVER-> */ /*15-061*/
#ifdef CONFIG_SND_SOC_ESXXX
		if (tapan_dai[w->shift].id == AIF1_PB ||
		    tapan_dai[w->shift].id == AIF2_PB)
			ret = remote_cfg_slim_rx(tapan_dai[w->shift].id);
			if (ret < 0)
...
/* SH_AUDIO_DRIVER-> */ /*15-061*/
		...
		if (tapan_dai[w->shift].id == AIF1_PB ||
		    tapan_dai[w->shift].id == AIF2_PB)
			ret = remote_close_slim_rx(tapan_dai[w->shift].id);
#endif
/* <-SH_AUDIO_DRIVER */ /*15-061*/
...
/* SH_AUDIO_DRIVER-> */ /*15-061*/
#ifdef CONFIG_SND_SOC_ESXXX
		if (tapan_dai[w->shift].id == AIF1_CAP)
			ret = remote_cfg_slim_tx(tapan_dai[w->shift].id);
#endif
/* <-SH_AUDIO_DRIVER */ /*15-061*/
		break;
	case SND_SOC_DAPM_POST_PMD:
...
#ifdef CONFIG_SND_SOC_ESXXX
		if (tapan_dai[w->shift].id == AIF1_CAP)
			ret = remote_close_slim_tx(tapan_dai[w->shift].id);
#endif
/* <-SH_AUDIO_DRIVER */ /*15-061*/
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-083 */
	perf_lock_init(&hkmode_a2dp_perf_lock,  PERF_LOCK_998400KHz, "hkmode_a2dp");
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 13-083 */

#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-089 */
	perf_lock_init(&playback_status_perf_lock,  PERF_LOCK_600000KHz, "playback_status");
	mutex_init(&playback_lock);
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 13-089 */
	return ret;
