...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-091 */
#include <linux/wakelock.h>
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 13-091 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-078 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-059 */
		snd_soc_write(mbhc->codec, WCD9XXX_A_MBHC_INSERT_DETECT,
			      (0xF0 | (ins ? (1 << 1) : 0)));
#else /* CONFIG_SH_AUDIO_DRIVER */ /* 13-059 */
		snd_soc_write(mbhc->codec, WCD9XXX_A_MBHC_INSERT_DETECT,
			      (0x68 | (ins ? (1 << 1) : 0)));
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 13-059 */
	else
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-059 */
		snd_soc_write(mbhc->codec, WCD9XXX_A_MBHC_INSERT_DETECT,
			      (0xF4 | (ins ? (1 << 1) : 0)));
...
#ifndef CONFIG_SH_AUDIO_DRIVER /* 13-078 */
...
#endif /* CONFIG_SH_AUDIO_DRIVER *//* 13-078 */
					 ...
					 enum wcd9xxx_mbhc_plug_type plug_type)
{
	...
	if (plug_type == PLUG_TYPE_HEADPHONE &&
		 ...
		 */
		if (mbhc->event_state &
#ifdef CONFIG_SH_AUDIO_DRIVER /*13-098*/
		(1 << MBHC_EVENT_PA_HPHL | 1 << MBHC_EVENT_PA_HPHR |
		 1 << MBHC_EVENT_PRE_TX_1_3_ON))
#else
		(1 << MBHC_EVENT_PA_HPHL | 1 << MBHC_EVENT_PA_HPHR))
#endif /* CONFIG_SH_AUDIO_DRIVER */ /*13-098*/
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-091 */
	wake_lock_init(&detec_wlock, WAKE_LOCK_SUSPEND, "mbhc_detect_resend");
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 13-091 */
	current_source_enable = (((mbhc->mbhc_cfg->cs_enable_flags &
	...
	while (!time_after(jiffies, timeout)) {
		...
		if (plug_type == PLUG_TYPE_INVALID) {
			...
			if (!mbhc->mbhc_cfg->detect_extn_cable &&
			    ...
			    mbhc->current_plug == PLUG_TYPE_NONE) {
#ifdef CONFIG_SH_AUDIO_DRIVER /*13-088*/
				WCD9XXX_BCL_LOCK(mbhc->resmgr);
#endif /* CONFIG_SH_AUDIO_DRIVER */ /*13-088*/
				wcd9xxx_report_plug(mbhc, 1,
						    SND_JACK_HEADPHONE);
#ifdef CONFIG_SH_AUDIO_DRIVER /*13-088*/
				WCD9XXX_BCL_UNLOCK(mbhc->resmgr);
...
irqreturn_t wcd9xxx_dce_handler(int irq, void *data)
{
	...
	void *calibration = mbhc->mbhc_cfg->calibration;
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-005 */
	int temp;
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 13-005 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-005 */
	if (mbhc->current_plug != PLUG_TYPE_HEADSET) {
		pr_err("test %s: plug type is %d. ignore\n", __func__, mbhc->current_plug);
		goto done;
	}
