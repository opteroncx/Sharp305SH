...
struct slim_device *sbdev_gol = NULL;
struct clk *aud_ext_clk = NULL;

int sc_clock_onoff(struct clk *clk, int sw)
{
	int ret = 0;
	dev_dbg(&sbdev_gol->dev, "%s(): Entry %d\n", __func__, sw);

	if (sw) {
		ret = clk_set_rate(clk, 19200000);
		if (ret < 0) {
			dev_err(&sbdev_gol->dev,
				"%s: AUD external Clock - clk_set_rate failed %d\n",
				__func__, ret);
			goto clk_set_rate_failed;
		}

		ret = clk_prepare(clk);
		if (ret) {
			dev_err(&sbdev_gol->dev,
				"%s: Failed to prepare AUD external Clock %d\n",
				__func__, ret);
			goto clk_prepare_failed;
		}

		ret = clk_enable(clk);
		if (ret) {
			dev_err(&sbdev_gol->dev,
				"%s: Failed to enable AUD external Clock %d\n",
				__func__, ret);
	...
	dev_dbg(&sbdev_gol->dev, "%s(): Leave\n", __func__);
	return 0;

clk_enable_failed:
	clk_unprepare(aud_ext_clk);
clk_prepare_failed:
clk_set_rate_failed:
	return ret;
}

int sc_clock_ctrl(int sw)
{
	int ret = 0;

	dev_dbg(&sbdev_gol->dev, "%s(): Entry\n", __func__);

	if (aud_ext_clk == NULL) {
		if (!sw) {
			dev_dbg(&sbdev_gol->dev,
				"%s(): AUD external already disable\n",
				__func__);
			return 0;
		}
		aud_ext_clk = clk_get(&sbdev_gol->dev, "cam_gp0_clk");
		if (IS_ERR_OR_NULL(aud_ext_clk)) {
			dev_err(&sbdev_gol->dev,
				"%s(): Can't find AUD external Clock\n",
				__func__);
			goto clk_get_failed;
		}
	} else if (sw) {
		dev_dbg(&sbdev_gol->dev, "%s(): AUD ecternal Clock already enable\n", __func__);
		return 0;
	}

	dev_dbg(&sbdev_gol->dev, "%s(): Got aud_cam_clk\n", __func__);

	ret = sc_clock_onoff(aud_ext_clk, sw);
	if (ret) {
		dev_err(&sbdev_gol->dev,
			"%s: Failed to %s AUD external Clock\n",
			__func__, sw == 1 ? "On" : "Off");
		goto onoff_clk_failed;
	}
...
static int escore_slim_probe(struct slim_device *sbdev)
{
	...
	if (sbdev->dev.of_node) {
		int aud_power_amp_gpio, aud_ext_clk_gpio;
		aud_power_amp_gpio = of_get_named_gpio(sbdev->dev.of_node,"adnc,power-gpio", 0);

		pr_info("%s: aud_power_amp_gpio = %d\n", __func__, aud_power_amp_gpio);

		if (aud_power_amp_gpio < 0) {
			dev_err(&sbdev->dev,
				"Looking up %s property in node %s failed %d\n",
				"adnc,power-gpio",
				sbdev->dev.of_node->full_name, aud_power_amp_gpio);
			return -EIO;
		} else {
			rc = gpio_request(aud_power_amp_gpio, "AUDIENCE_POWER_EN_GPIO");
			if (rc) {
				dev_err(&sbdev->dev,
					"%s: Failed to request audience power gpio %d rc:%d\n",
					__func__, aud_power_amp_gpio, rc);
				return rc;
			}
			/* Enable the power */
			gpio_direction_output(aud_power_amp_gpio, 1);
		}

		aud_ext_clk_gpio = of_get_named_gpio(sbdev->dev.of_node,"adnc,extclk-gpio", 0);

		pr_info("%s: aud_ext_clk_gpio = %d\n", __func__, aud_ext_clk_gpio);

		if (aud_ext_clk_gpio < 0) {
			dev_err(&sbdev->dev,
				"Looking up %s property in node %s failed %d\n",
				"adnc,extclk-gpio",
				sbdev->dev.of_node->full_name, aud_ext_clk_gpio);
			return -EIO;
		} else {
			rc = gpio_request(aud_ext_clk_gpio, "AUDIENCE_EXT_CLK_GPIO");
			if (rc) {
				dev_err(&sbdev->dev,
					"%s: Failed to request audience ext clk gpio %d rc:%d\n",
					__func__, aud_ext_clk_gpio, rc);
				return rc;
			}
		}
	}
...
int escore_slimbus_init()
{
	...
	if (!rc)
		pr_debug("%s() registered as SLIMBUS\n", __func__);
