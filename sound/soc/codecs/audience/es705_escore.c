...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 15-061 */
#include <linux/wakelock.h>
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 15-061 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 15-061 */
extern	int sc_clock_ctrl(int sw);

#define ES705_PRESET_ARRAY_SIZE 10
u16 Preset_array[ES705_PRESET_ARRAY_SIZE] = {0};
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 15-061 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 15-061 */
			.channels_max = 3,
#else /* CONFIG_SH_AUDIO_DRIVER */ /* 15-061 */
			.channels_max = 2,
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 15-061 */
			.rates = ES705_SLIMBUS_RATES,
			.formats = ES705_SLIMBUS_FORMATS,
...
#ifdef CONFIG_ARCH_MSM8926 /*Please make sure it it valid */
/*Slimbus channel map for MSM8926*/
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 15-061 */
	escore_priv->slim_dai_data[DAI_INDEX(ES_SLIM_1_CAP)].ch_num[2] =
						es705_slim_tx_port_to_ch[6];
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 15-061 */
	/* back end for TX1 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 15-061 */
	 escore_priv->slim_dai_data[DAI_INDEX(ES_SLIM_3_PB)].ch_num[2] =
						es705_slim_rx_port_to_ch[6];
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 15-061 */
	/* front end for RX2 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 15-061 */
	if (escore_priv.dev ==  NULL) {
		dev_dbg(escore_priv.dev, "%s(): Can't detect es804, disable clk.\n",
			__func__);
		sc_clock_ctrl(0);
		return 0;
	}
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 15-061 */
static int es705_put_preset_array_value(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_dbg(escore_priv.dev, "%s(): Entry\n", __func__);
	mutex_lock(&escore_priv.overall_mutex);

	for(count = 0; count < ES705_PRESET_ARRAY_SIZE; count++)
	{
		Preset_array[count] = ucontrol->value.integer.value[count];

		if(ucontrol->value.integer.value[count] == 0)
			break;

		rc = escore_write(NULL, reg, Preset_array[count] );

		if (rc) {
			dev_err(escore_priv.dev, "%s(): Set Preset failed\n",
				__func__);
			mutex_unlock(&escore_priv.overall_mutex);
			return rc;
		};

		dev_dbg(escore_priv.dev, "%s(): Set Preset Value Preset_array[%d] = %d\n",
			__func__, count, Preset_array[count]);
		usleep(1000);
	}

	mutex_unlock(&escore_priv.overall_mutex);
	dev_dbg(escore_priv.dev, "%s(): Level\n", __func__);
	return rc;
}
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 15-061 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 15-061 */
static int es705_get_preset_array_value(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	unsigned int count;
	...
	for(count = 0; count < ES705_PRESET_ARRAY_SIZE; count++)
	{
		ucontrol->value.integer.value[count] = Preset_array[count];
	}
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 15-061 */
	SOC_SINGLE_MULTI_EXT("Preset Array",
		       ES705_PRESET, 0, 65535, 0, ES705_PRESET_ARRAY_SIZE, es705_get_preset_array_value,
		       es705_put_preset_array_value),
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 15-061 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /*15-061*/
	dev_dbg(codec->dev, "%s() -kthread- start\n", __func__);
#endif /* CONFIG_SH_AUDIO_DRIVER */ /*15-061*/
...
#ifdef CONFIG_SH_AUDIO_DRIVER /*15-061*/
	dev_dbg(codec->dev, "%s() -kthread- end\n", __func__);
#endif /* CONFIG_SH_AUDIO_DRIVER */ /*15-061*/
	return rc;
}
...
#ifdef CONFIG_SH_AUDIO_DRIVER /*15-061*/
	pdata->esxxx_clk_cb = sc_clock_ctrl;
#else /* CONFIG_SH_AUDIO_DRIVER */ /* 15-061 */
...
#if defined(CONFIG_MQ100_SENSOR_HUB)
	const char *fw_filename = "audience-mq100-i2c-fw.bin";
	const char *vs_filename = "audience-mq100-i2c-fw.bin";
	/* FIXME: dummy bkg added until actual bkg is used */
	const char *bkg_filename = "audience-mq100-i2c-fw.bin";
#else
	const char *fw_filename = "aud.bin";
...
*/
	...
	if (pdata->gpioa_gpio != -1) {
		...
		if (rc < 0) {
			dev_err(escore_priv.dev,
				"%s(): es705_gpioa request failed", __func__);
			goto gpioa_gpio_request_error;
		}
		rc = gpio_direction_input(pdata->gpioa_gpio);
		if (rc < 0) {
			dev_err(escore_priv.dev,
				"%s(): es705_gpioa direction failed", __func__);
	...
	if (pdata->gpiob_gpio != -1) {
		...
		if (rc < 0) {
			dev_err(escore_priv.dev,
				"%s(): es705_gpiob request failed", __func__);
			goto gpiob_gpio_request_error;
		}
		rc = gpio_direction_input(pdata->gpiob_gpio);
		if (rc < 0) {
			dev_err(escore_priv.dev,
				"%s(): es705_gpiob direction failed", __func__);
...
#ifdef CONFIG_SH_AUDIO_DRIVER /*15-061*/
	dev_dbg(escore_priv.dev, "%s(): firmware:%s\n", __func__, fw_filename);
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 15-061 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /*15-061*/
	dev_dbg(escore_priv.dev, "%s(): vs_firmware:%s\n", __func__, vs_filename);
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 15-061 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /*15-061*/
	dev_dbg(escore_priv.dev, "%s(): bkg_firmware:%s\n", __func__, bkg_filename);
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 15-061 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 15-061 */
	mutex_init(&escore_priv.overall_mutex);
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 15-061 */
