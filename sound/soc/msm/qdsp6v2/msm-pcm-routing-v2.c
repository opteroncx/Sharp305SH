...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-089 */
static int music_last_status = 0;
#endif /* CONFIG_SH_AUDIO_DRIVER *//* 13-089 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-089 */
	if (msm_routing_get_is_music_play()) {
		if (music_last_status == 0) {
			music_last_status = 1;
			tapan_codec_set_playback_status(music_last_status, 2);
		}
		pr_debug("%s: Music is playing", __func__);
	} else if (music_last_status) {
		music_last_status = 0;
		tapan_codec_set_playback_status(music_last_status, 2);
		pr_debug("%s: Music is stoping", __func__);
	}
#endif /* CONFIG_SH_AUDIO_DRIVER *//* 13-089 */
