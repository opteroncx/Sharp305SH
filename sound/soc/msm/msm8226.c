...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-004 */
#include <sharp/shspamp.h>
#ifdef CONFIG_SH_AUDIO_SMARTAMP /* 15-002 */
#include <sharp/shsmartamp_ssm4329.h>
#endif /* CONFIG_SH_AUDIO_SMARTAMP */ /* 15-002 */
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 13-004 */
...
#define ON 1
#define OFF 0
static int msm8226_ext_pr_pamp;
static int msm8226_spk_control=0;
static int msm8226_pr_control=0;
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 13-004 */
...
#ifndef CONFIG_SH_AUDIO_SMARTAMP /* 15-002 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 15-002 */
#ifdef CONFIG_SH_AUDIO_SMARTAMP
static void msm8926_spk_power_smartamp_control(u32 on)
{
...
static void msm8226_ext_spk_power_amp_on(u32 spk)
{
	...
	if (spk ) {
		...
		if (msm8226_ext_spk_pamp) {
			...
			msm8926_spk_power_smartamp_control(1);
		}
	} else  {
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-004 */
		if (spk ) {
#else /* CONFIG_SH_AUDIO_DRIVER */ /* 13-004 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-003 */
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-004 */
			if (msm8226_ext_spk_pamp) {
#else /* CONFIG_SH_AUDIO_DRIVER */ /* 13-004 */
...
#else	/* CONFIG_SH_AUDIO_DRIVER */ /* 13-003 */
			if ((msm8226_ext_spk_pamp & LO_1_SPK_AMP) &&
...
#endif	/* CONFIG_SH_AUDIO_DRIVER */ /* 13-003 */
		} else  {
			pr_err("%s: Invalid external speaker ampl. spk = 0x%x\n",
				__func__, spk);
		}
	...
	if (!spk) {
		...
		if (!msm8226_ext_spk_pamp) {
			if (ext_spk_amp_gpio >= 0) {
				...
				msm8926_spk_power_smartamp_control(0);
			}
		}
	 } else  {
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-004 */
static void msm8226_ext_pr_power_amp_on(u32 pr)
{
	if (pr) {
		pr_debug("%s:Enable left and right panel receiver case pr = 0x%x\n",
			__func__, pr);
		msm8226_ext_pr_pamp |= pr;

		if (msm8226_ext_pr_pamp) {
			pr_debug("%s(): enable power\n", __func__);
			shreceiver_set_receiver(1);
		}
	} else {
		pr_err("%s: Invalid external panel receiver ampl. pr = 0x%x\n",
			__func__, pr);
	}
}

static void msm8226_ext_pr_power_amp_off(u32 pr)
{
	if (!pr) {
		pr_debug("%s Disable left and right panel receiver case pr = 0x%08x\n",
			__func__, pr);
		...
		pr_err("%s: ERROR : Invalid Ext pr Ampl. pr = 0x%08x\n",
			__func__, pr);
	}
}
#else /* CONFIG_SH_AUDIO_DRIVER */ /* 13-004 */
...
/* SH_AUDIO_DRIVER -> */ /* 13-001 */
	SND_SOC_DAPM_MIC("Secondary Mic", NULL),
/* SH_AUDIO_DRIVER <- */ /* 13-001 */
/* SH_AUDIO_DRIVER -> */ /* 15-098 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-004 */
static const char *const panel_receiver_function_text[] = {"Off", "On"};
static const char *const speaker_function_text[] = {"Off", "On"};
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 13-004 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-004 */
static const struct soc_enum msm_switch_enum[] = {
	SOC_ENUM_SINGLE_EXT(2, speaker_function_text),
	SOC_ENUM_SINGLE_EXT(2, panel_receiver_function_text),
};

static int msm8226_set_spk(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s()\n", __func__);
	if (msm8226_spk_control == ucontrol->value.integer.value[0])
		return 0;

	msm8226_spk_control = ucontrol->value.integer.value[0];

	if(msm8226_spk_control)
		msm8226_ext_spk_power_amp_on(ON);
	else
		msm8226_ext_spk_power_amp_off(OFF);
	return 1;
}

static int msm8226_get_spk(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s: msm8226_spk_control = %d", __func__, msm8226_spk_control);
	ucontrol->value.integer.value[0] = msm8226_spk_control;
	return 0;
}

static int msm8226_set_pr(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s(%ld)  %d\n", __func__,ucontrol->value.integer.value[0], msm8226_pr_control);
	if (msm8226_pr_control == ucontrol->value.integer.value[0])
		return 0;

	msm8226_pr_control = ucontrol->value.integer.value[0];

	if(msm8226_pr_control)
		msm8226_ext_pr_power_amp_on(ON);
	else
		msm8226_ext_pr_power_amp_off(OFF);
	return 1;
}

static int msm8226_get_pr(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s: msm8226_pr_control = %d\n", __func__, msm8226_pr_control);
	ucontrol->value.integer.value[0] = msm8226_pr_control;
	return 0;
}
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 13-004 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 13-004 */
	SOC_ENUM_EXT("Speaker switch", msm_switch_enum[0],
			msm8226_get_spk, msm8226_set_spk),
	SOC_ENUM_EXT("Panel receiver switch", msm_switch_enum[1],
			msm8226_get_pr, msm8226_set_pr),
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 13-004 */
