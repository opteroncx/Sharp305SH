...
DEFINE_MSM_MUTEX(bu64241_act_mutex);
...
static struct msm_actuator bu64241_act_table;

static struct i2c_driver bu64241_act_i2c_driver;
static struct msm_actuator *actuators[] = {
	&bu64241_act_table,
};

static   uint16_t local_init_setting_size = 4;
static   struct reg_settings_t local_init_settings[] = {
	        {0xCC, 0x23}, // mod
	        {0xD4, 0x01}, // mod
	        {0xDF, 0xFF}, // mod
	        {0xE4, 0x81}, // mod
...
static void bu64241_act_parse_i2c_params(struct msm_actuator_ctrl_t *a_ctrl,
...
static int32_t bu64241_act_init_focus(struct msm_actuator_ctrl_t *a_ctrl,
...
static void bu64241_act_write_focus(
...
static int32_t bu64241_act_move_focus(
	struct msm_actuator_ctrl_t *a_ctrl,
...
static int32_t bu64241_act_init_step_table(struct msm_actuator_ctrl_t *a_ctrl,
...
static int32_t bu64241_act_set_default_focus(
	struct msm_actuator_ctrl_t *a_ctrl,
	struct msm_actuator_move_params_t *move_params)
{
	int32_t rc = 0;
...
static int32_t bu64241_act_power_down(struct msm_actuator_ctrl_t *a_ctrl)
{
	int32_t rc = 0;
	CDBG("Enter\n");
#if 0
...
static int32_t bu64241_act_init(struct msm_actuator_ctrl_t *a_ctrl,
	struct msm_actuator_set_info_t *set_info) {
	struct reg_settings_t *init_settings = NULL;
	int32_t rc = -EFAULT;
	uint16_t i = 0;
	struct msm_camera_cci_client *cci_client = NULL;
	unsigned short hw_rev = SH_REV_PP1;
	CDBG("Enter\n");
...
//	hw_rev = sh_boot_get_hw_revision();
	...
	if (local_init_setting_size) {
		if (a_ctrl->func_tbl->actuator_init_focus) {
			init_settings = kmalloc(sizeof(struct reg_settings_t) *
				(local_init_setting_size),
				GFP_KERNEL);
			...
			memcpy(init_settings,&local_init_settings[0], local_init_setting_size * sizeof(struct reg_settings_t));
...
static int32_t bu64241_act_i2c_write(struct msm_actuator_ctrl_t *a_ctrl,
	void __user *argp)
{
	...
	if( cdata->cfg.i2c_info.length > 1 ){
		/* not support */
...
static int32_t bu64241_act_i2c_read(struct msm_actuator_ctrl_t *a_ctrl,
	void __user *argp)
{
	struct msm_actuator_cfg_data *cdata = (struct msm_actuator_cfg_data *)argp;
	int32_t rc = -EFAULT;
	...
	rc = a_ctrl->i2c_client.i2c_func_tbl->i2c_read(
			&a_ctrl->i2c_client,
			cdata->cfg.i2c_info.addr,
			&readdata, MSM_CAMERA_I2C_WORD_DATA);
	if (rc < 0)
		return -EFAULT;
	
	data[0] = (uint8_t)((readdata&0xFF00)>>8);
	data[1] = (uint8_t)(readdata&0x00FF);
	CDBG_DEB("%s read data[0] = 0x%0x data[1] = 0x%0x\n",   __func__, data[0], data[1]);
...
static int32_t bu64241_act_config(struct msm_actuator_ctrl_t *a_ctrl,
	void __user *argp)
{
	...
	case CFG_SET_ACTUATOR_INFO:
		/* no reload */
		if( a_ctrl->step_position_table == NULL){
			rc = bu64241_act_init(a_ctrl, &cdata->cfg.set_info);
		if (rc < 0)
	...
	case SHCFG_GET_I2C_DATA:
		rc = bu64241_act_i2c_read(a_ctrl, argp);
		if (rc < 0)
			pr_err("get i2c failed %d\n", rc);
		break;
		
	case SHCFG_SET_I2C_DATA:
		rc = bu64241_act_i2c_write(a_ctrl, argp);
		if (rc < 0)
...
static int32_t bu64241_act_get_subdev_id(struct msm_actuator_ctrl_t *a_ctrl,
	void *arg)
{
...
static int bu64241_act_open(struct v4l2_subdev *sd,
...
static int bu64241_act_close(struct v4l2_subdev *sd,
	struct v4l2_subdev_fh *fh) {
	int rc = 0;
	struct msm_actuator_ctrl_t *a_ctrl =  v4l2_get_subdevdata(sd);
	CDBG("Enter\n");
...
static const struct v4l2_subdev_internal_ops bu64241_act_internal_ops = {
	.open = bu64241_act_open,
	.close = bu64241_act_close,
};

static long bu64241_act_subdev_ioctl(struct v4l2_subdev *sd,
			unsigned int cmd, void *arg)
{
	...
	case VIDIOC_MSM_SENSOR_GET_SUBDEV_ID:
		return bu64241_act_get_subdev_id(a_ctrl, argp);
	case VIDIOC_MSM_ACTUATOR_CFG:
		return bu64241_act_config(a_ctrl, argp);
	case MSM_SD_SHUTDOWN:
		bu64241_act_close(sd, NULL);
		return 0;
	default:
		return -ENOIOCTLCMD;
	}
...
static int32_t bu64241_act_power_up(struct msm_actuator_ctrl_t *a_ctrl)
{
	int rc = 0;
	CDBG("%s called\n", __func__);
...
static int32_t bu64241_act_power(struct v4l2_subdev *sd, int on)
{
	int rc = 0;
	struct msm_actuator_ctrl_t *a_ctrl = v4l2_get_subdevdata(sd);
	CDBG("Enter\n");
	mutex_lock(a_ctrl->actuator_mutex);
	if (on)
		rc = bu64241_act_power_up(a_ctrl);
	else
		rc = bu64241_act_power_down(a_ctrl);
	mutex_unlock(a_ctrl->actuator_mutex);
	CDBG("Exit\n");
	return rc;
}

static struct v4l2_subdev_core_ops bu64241_act_subdev_core_ops = {
	.ioctl = bu64241_act_subdev_ioctl,
	.s_power = bu64241_act_power,  /* not called in normal power on/off sequence */
};

static struct v4l2_subdev_ops bu64241_act_subdev_ops = {
	.core = &bu64241_act_subdev_core_ops,
};

static const struct i2c_device_id bu64241_act_i2c_id[] = {
	{"bu64241,actuator", (kernel_ulong_t)NULL},
	{ }
};

static int32_t bu64241_act_i2c_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	...
	if (client == NULL) {
		pr_err("bu64241_act_i2c_probe: client is null\n");
		rc = -EINVAL;
		goto probe_failure;
	}
	...
	act_ctrl_t->i2c_driver = &bu64241_act_i2c_driver;
	...
	act_ctrl_t->act_v4l2_subdev_ops = &bu64241_act_subdev_ops;
	act_ctrl_t->actuator_mutex = &bu64241_act_mutex;
	...
	act_ctrl_t->msm_sd.sd.internal_ops = &bu64241_act_internal_ops;
	...
	pr_info("bu64241_act_i2c_probe: succeeded\n");
	CDBG("Exit\n");
...
static int32_t bu64241_act_platform_probe(struct platform_device *pdev)
{
	int32_t rc = 0;
	struct msm_camera_cci_client *cci_client = NULL;
	...
	msm_actuator_t->act_v4l2_subdev_ops = &bu64241_act_subdev_ops;
	msm_actuator_t->actuator_mutex = &bu64241_act_mutex;
	...
	msm_actuator_t->msm_sd.sd.internal_ops = &bu64241_act_internal_ops;
	msm_actuator_t->msm_sd.sd.flags |= V4L2_SUBDEV_FL_HAS_DEVNODE;
	snprintf(msm_actuator_t->msm_sd.sd.name,
		ARRAY_SIZE(msm_actuator_t->msm_sd.sd.name), "bu64241_act");
...
static const struct of_device_id bu64241_act_i2c_dt_match[] = {
	{.compatible = "bu64241,actuator"},
	{}
};

MODULE_DEVICE_TABLE(of, bu64241_act_i2c_dt_match);

static struct i2c_driver bu64241_act_i2c_driver = {
	.id_table = bu64241_act_i2c_id,
	.probe  = bu64241_act_i2c_probe,
	.remove = __exit_p(bu64241_act_i2c_remove),
	.driver = {
		.name = "bu64241,actuator",
		.owner = THIS_MODULE,
		.of_match_table = bu64241_act_i2c_dt_match,
	},
};

static const struct of_device_id bu64241_act_dt_match[] = {
	{.compatible = "bu64241,actuator", .data = NULL},
	{}
};

MODULE_DEVICE_TABLE(of, bu64241_act_dt_match);

static struct platform_driver bu64241_act_platform_driver = {
	.driver = {
		.name = "bu64241,actuator",
		.owner = THIS_MODULE,
		.of_match_table = bu64241_act_dt_match,
	},
};

static int __init bu64241_act_init_module(void)
{
	int32_t rc = 0;
	CDBG("Enter\n");
	rc = platform_driver_probe(&bu64241_act_platform_driver,
		bu64241_act_platform_probe);
	if (!rc)
		return rc;
	CDBG("%s:%d rc %d\n", __func__, __LINE__, rc);
	return i2c_add_driver(&bu64241_act_i2c_driver);
}

static struct msm_actuator bu64241_act_table = {
	.act_type = ACTUATOR_VCM,
	.func_tbl = {
		.actuator_init_step_table = bu64241_act_init_step_table,
		.actuator_move_focus = bu64241_act_move_focus,
		.actuator_write_focus = bu64241_act_write_focus,
		.actuator_set_default_focus = bu64241_act_set_default_focus,
		.actuator_init_focus = bu64241_act_init_focus,
		.actuator_parse_i2c_params = bu64241_act_parse_i2c_params,
	},
};

module_init(bu64241_act_init_module);
MODULE_DESCRIPTION("bu64241 ACTUATOR");
