...
#ifdef CONFIG_SHSYS_CUST
#define DEF_FREQ_STEP					(17)
#define DEF_FREQ_STEP_HIGH				(9)
#define NR_CORE_INPUT_EVENT				0x03 /* 0011 */
#define MIGRATION_FREQ				(1190400)
#define SAMPLING_LATE_MDSS_MDP_CLK_ENABLE	(20000)
#define SHSYS_CPUFREQ_ONDEMAND_BOOST			(1190400)
#endif /* CONFIG_SHSYS_CUST */
...
static void sh_cpufreq_ondemand_boost(void);

static ssize_t store_sh_boost(struct kobject *a, struct attribute *b,
				   const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	dbs_tuners_ins.sh_boost = val;

	if (dbs_tuners_ins.sh_boost) {
		sh_boost_endtime = ktime_to_us(ktime_get()) + sh_boost_duration_val;
		sh_cpufreq_ondemand_boost();
	}

	dbs_tuners_ins.sh_boost = 0;
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	if ((max_load_freq > dbs_tuners_ins.up_threshold * policy->cur) ||
        (sh_boost_endtime > ktime_to_us(ktime_get()))) {
		int target;
	...
	if (policy->cur > policy->min &&
		dbs_tuners_ins.optimal_freq > policy->cur &&
		max_load_freq > (dbs_tuners_ins.up_threshold_multi_core - dbs_tuners_ins.down_differential) * policy->cur) {
		dbs_freq_increase(policy, dbs_tuners_ins.optimal_freq);
		return;
	} else if(policy->cur == policy->min &&
			max_load_freq > dbs_tuners_ins.up_threshold_multi_core * policy->cur) {
		dbs_freq_increase(policy, dbs_tuners_ins.optimal_freq);
		return;
	}
#endif /* CONFIG_SHSYS_CUST */
...
static void do_dbs_timer(struct work_struct *work)
{
	...
	if (!dbs_tuners_ins.powersave_bias ||
		...
		if (dbs_info->freq_lo) {
			...
			if(mdp_clock_enable || kgsl_is_working() || msm_routing_get_is_music_play()){
				delay = usecs_to_jiffies(SAMPLING_LATE_MDSS_MDP_CLK_ENABLE * dbs_info->rate_mult);
				pr_debug("cpu%d : sampling rate -> %d us, mdp_clk = %d gpu_clk = %d is_music_play = %d check clock up elapsed time \n", cpu, (int)(SAMPLING_LATE_MDSS_MDP_CLK_ENABLE*dbs_info->rate_mult),  mdp_clock_enable, kgsl_is_working(), msm_routing_get_is_music_play());
			}else{
...
static void sh_cpufreq_ondemand_boost(void)
{
	...
	struct cpu_dbs_info_s *this_dbs_info;
	struct cpufreq_policy *policy;
	target_freq = SHSYS_CPUFREQ_ONDEMAND_BOOST;
...
static int is_clock_boost(unsigned int type, unsigned int code)
{
	if (type == EV_KEY) {
		switch (code) {
		case KEY_POWER:
