...
void mdss_dsi_host_init(struct mdss_panel_data *pdata)
{
	...
	/* DSI_ERR_INT_MASK0 */
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00042 */
	MIPI_OUTP((ctrl_pdata->ctrl_base) + 0x010c, 0x11ff3fe0);/* BIT(25) DLN0_ERR_CONTENTION_LP1_MASK */
#else
...
#ifdef CONFIG_SHLCDC_BOARD		/* CUST_ID_00041 */
	if (rlen <= 2) {
#else
	if (rlen == 0) {
#endif
...
#ifndef CONFIG_SHLCDC_BOARD		/* CUST_ID_00041 */
		 if (!short_response) {
#endif
...
void mdss_dsi_dln0_phy_err(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	if (status & 0x011111) {
		...
		if (status & 0x010000) {
			...
			mdss_dsi_dln0_err_contention_lp1_mask(1);/* disable */

			ret = mdss_shdisp_lcd_recovery();
			if (ret) {
				mdss_dsi_dln0_err_contention_lp1_mask(0);/* enable */
				pr_err("%s: mdss_shdisp_lcd_recovery failed !\n", __func__);
			}
		} else {
			MIPI_OUTP(base + 0x00b4, status);
			pr_err("%s: status=%x\n", __func__, status);
