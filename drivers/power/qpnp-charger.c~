...
#define SHCHG_ENABLE_EXTERNAL_OVP_FET
#endif /* CONFIG_BATTERY_SH */

//#define SHCHG_TORCH_FAILSAFE
...
#ifdef SHCHG_ENABLE_EXTERNAL_OVP_FET
#define MISC_RAW_DVDD_RB_SCRATCH	0xCE
#endif /* SHCHG_ENABLE_EXTERNAL_OVP_FET */
#endif /* CONFIG_BATTERY_SH */
...
#define PMIC_GPIO_BASE	440
#define PMIC_GPIO_PM_TO_SYS(pm_gpio)		(pm_gpio - 1 + PMIC_GPIO_BASE)

#define SET_PMIC_GPIO(gpio,v)	((__gpio_cansleep(gpio) == 0) ? gpio_set_value(gpio, v) : gpio_set_value_cansleep(gpio, v))
#define GET_PMIC_GPIO(gpio)		((__gpio_cansleep(gpio) == 0) ? gpio_get_value(gpio)    : gpio_get_value_cansleep(gpio)   )
#endif /* CONFIG_BATTERY_SH */
...
struct qpnp_chg_chip {
	...
	u8						iusb_trim;
	u8						idc_trim;
	int						torch_control_status;
	bool					torch_control_detect;
	struct delayed_work		torch_check_work;
	struct mutex			torch_control_lock;
	struct delayed_work		torch_safety_work;
#endif /* CONFIG_BATTERY_SH */
};
...
#define QPNP_TORCH_CHECK_WORK_MSEC	1500
#define QPNP_TORCH_SAFETY_WORK_MSEC	10000

#define QPNP_TORCH_CONTROL_OFF			0
#define QPNP_TORCH_CONTROL_ON			1
#define QPNP_TORCH_CONTROL_OFF_NO_ICHG	2
#define QPNP_TORCH_CONTROL_DETECT		3
static int qpnp_torch_check_msec = QPNP_TORCH_CHECK_WORK_MSEC;
module_param_named(torch_check_msec, qpnp_torch_check_msec, int, 0600);
static int qpnp_torch_safety_msec = QPNP_TORCH_SAFETY_WORK_MSEC;
module_param_named(torch_safety_msec, qpnp_torch_safety_msec, int, 0600);

static int qpnp_torch_check_susp = -1;
module_param_named(torch_check_susp, qpnp_torch_check_susp, int, 0600);
static int __qpnp_torch_control_enable(bool enable, bool safefy);
#endif /* CONFIG_BATTERY_SH */
...
qpnp_chg_is_boost_en_set(struct qpnp_chg_chip *chip)
{
	...
	if (chip->torch_control_status != QPNP_TORCH_CONTROL_OFF)
	{
		boost_en_ctl &= ~BOOST_PWR_EN;
		pr_debug("torch control: status=%d boost_en=0x%02x\n",
			chip->torch_control_status, boost_en_ctl);
	}
#endif /* CONFIG_BATTERY_SH */
...
static int
qpnp_chg_is_vdd_loop_active(struct qpnp_chg_chip *chip)
{
	u8 buck_sts;
	int rc;
	...
	return (buck_sts & VDD_LOOP_IRQ) ? 1 : 0;
}
...
#ifdef CONFIG_BATTERY_SH
#if defined(CONFIG_ARCH_MSM8974) && defined(CONFIG_LEDS_QPNP)
	/* Boost Bypass set to disable  */
	qpnp_regulator_boost_bypass_enable( 0 );
#endif	/* CONFIG_ARCH_MSM8974 && CONFIG_LEDS_QPNP */
#endif	/* CONFIG_BATTERY_SH */
...
qpnp_chg_regulator_boost_disable(struct regulator_dev *rdev)
{
	...
	{
		rc = 0;
		pr_debug("torch control: status=%d\n", chip->torch_control_status);
		goto skip;
	}
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
#ifdef SHCHG_ENABLE_EXTERNAL_OVP_FET
static int iusb_trim_store_data;
static bool iusb_trim_store_flg = false;
static int qpnp_chg_charger_iusb_trim_store( void )
{
	int rc = 0;
	u8 temp_reg = 0;
	...
	rc = qpnp_chg_read(the_chip, &temp_reg,
			the_chip->misc_base + MISC_RAW_DVDD_RB_SCRATCH, 1);
	if (rc) {
	...
	if (temp_reg)
	{
		pr_info("iusb_trim is stored.\n");
	}
	else
	{
		temp_reg = qpnp_chg_iusb_trim_get(the_chip);
		rc = qpnp_chg_write(the_chip, &temp_reg,
				the_chip->misc_base + MISC_RAW_DVDD_RB_SCRATCH, 1);
		if (rc) {
	...
	iusb_trim_store_data = temp_reg;
	pr_info("iusb_trim_store_data = 0x%02x\n", iusb_trim_store_data);

	iusb_trim_store_flg = true;

	return 0;
}

int qpnp_chg_charger_iusb_trim_restore( void )
{
	if (!the_chip)
	{
		pr_err("qpnp charger is not initialized\n");
	...
	if (iusb_trim_store_flg == false)
	{
		pr_err("iusb_trim is not stored.\n");
		return -EINVAL;
	}

	return qpnp_chg_iusb_trim_set(the_chip, iusb_trim_store_data);
}
EXPORT_SYMBOL_GPL(qpnp_chg_charger_iusb_trim_restore);

int qpnp_chg_charger_iusb_trim_set(int trim)
{
	if (!the_chip)
	{
		pr_err("qpnp charger is not initialized\n");
	...
	return qpnp_chg_iusb_trim_set(the_chip, trim);
}
EXPORT_SYMBOL_GPL(qpnp_chg_charger_iusb_trim_set);

int qpnp_chg_charger_iusb_trim_get(int* trim)
{
	int rc = 0;
	...
	*trim = qpnp_chg_iusb_trim_get(the_chip);

	return rc;
}
EXPORT_SYMBOL_GPL(qpnp_chg_charger_iusb_trim_get);

int qpnp_chg_charger_ichg_loop_get(bool* il_p)
{
	int rc = 0;
	...
	*il_p = qpnp_chg_is_ichg_loop_active(the_chip);

	return rc;
}
EXPORT_SYMBOL_GPL(qpnp_chg_charger_ichg_loop_get);

int qpnp_chg_charger_vchg_loop_get(bool* vl_p)
{
	int rc = 0;
	...
	*vl_p = get_prop_vchg_loop(the_chip);

	return rc;
}
EXPORT_SYMBOL_GPL(qpnp_chg_charger_vchg_loop_get);

int qpnp_chg_charger_ibat_loop_get(bool* il_p)
{
	int rc = 0;
	...
	*il_p = qpnp_chg_is_ibat_loop_active(the_chip);

	return rc;
}
EXPORT_SYMBOL_GPL(qpnp_chg_charger_ibat_loop_get);

int qpnp_chg_charger_vdd_loop_get(bool* vl_p)
{
	int rc = 0;
	...
	*vl_p = qpnp_chg_is_vdd_loop_active(the_chip);

	return rc;
}
EXPORT_SYMBOL_GPL(qpnp_chg_charger_vdd_loop_get);
#endif /* SHCHG_ENABLE_EXTERNAL_OVP_FET */
...
static int __qpnp_flash_control_enable(bool enable, bool safety)
{
	...
	if (enable)
	{
		SET_PMIC_GPIO(PMIC_GPIO_PM_TO_SYS(2), 0);
	...
	else
	{
		...
		{
			shbatt_cable_status_t cable_status = SHBATT_CABLE_STATUS_NONE;
			if(shbatt_api_get_charger_cable_status(&cable_status) != SHBATT_RESULT_SUCCESS)
			{
				pr_err("cannot get cable status\n");
			}

			if(cable_status == SHBATT_CABLE_STATUS_AC)
				SET_PMIC_GPIO(PMIC_GPIO_PM_TO_SYS(2), 1);
			else
				SET_PMIC_GPIO(PMIC_GPIO_PM_TO_SYS(2), 0);
		}
...
#ifdef SHCHG_TORCH_FAILSAFE
static void qpnp_torch_safety_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct qpnp_chg_chip *chip = container_of(dwork,
				struct qpnp_chg_chip, torch_safety_work);

	pr_err("status = %d torch control safety disable\n", chip->torch_control_status);
	__qpnp_torch_control_enable(false, true);
}
#endif /* SHCHG_TORCH_FAILSAFE */

static void qpnp_torch_status_change(struct qpnp_chg_chip *chip, int status)
{
    if (status == QPNP_TORCH_CONTROL_ON)
	{
		chip->torch_control_detect = true;
	}

	pr_debug("status = %d -> %d\n", chip->torch_control_status, status);
	chip->torch_control_status = status;
}

static void qpnp_torch_check_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct qpnp_chg_chip *chip = container_of(dwork,
				struct qpnp_chg_chip, torch_check_work);

	qpnp_torch_status_change(chip, QPNP_TORCH_CONTROL_OFF);

	shbatt_api_notify_power_supply_changed(SHBATT_PS_CATEGORY_BATTERY);
}

int __qpnp_torch_control_enable(bool enable, bool safety)
{
    int usb_susp_en;
	u8 reg;
	int rc = 0;
	...
	mutex_lock(&the_chip->torch_control_lock);

	if (!safety)
	{
#ifdef SHCHG_TORCH_FAILSAFE
		cancel_delayed_work_sync(&the_chip->torch_safety_work);
#endif /* SHCHG_TORCH_FAILSAFE */
	}
	...
	if (enable)
	{
		...
		usb_susp_en = (reg & USB_SUSPEND_BIT) ? 1 : 0;
		pr_debug("usb_susp read 0x%02x enable = %d\n", reg, usb_susp_en);
		qpnp_torch_check_susp = usb_susp_en;
		...
		cancel_delayed_work_sync(&the_chip->torch_check_work);
		qpnp_torch_status_change(the_chip, QPNP_TORCH_CONTROL_ON);

#ifdef SHCHG_TORCH_FAILSAFE
		schedule_delayed_work(&the_chip->torch_safety_work,
			msecs_to_jiffies(qpnp_torch_safety_msec));
#endif /* SHCHG_TORCH_FAILSAFE */
	...
	{
		if (the_chip->torch_control_status != QPNP_TORCH_CONTROL_ON)
		{
			pr_debug("disable skip status = %d\n", the_chip->torch_control_status);
			goto unlock;
		}
		...
		usb_susp_en = qpnp_torch_check_susp;
		if (shchg_api_is_charge_ignore())
		{
			pr_debug("charge ignore: usb_susp_en = %d\n", usb_susp_en);
			usb_susp_en = 1;
		...
		qpnp_torch_status_change(the_chip, QPNP_TORCH_CONTROL_OFF_NO_ICHG);
		schedule_delayed_work(&the_chip->torch_check_work,
			msecs_to_jiffies(qpnp_torch_check_msec));
	}

unlock:
	mutex_unlock(&the_chip->torch_control_lock);

	return rc;
}

int qpnp_torch_control_enable(bool enable)
{
	bool safety = false;
	return __qpnp_torch_control_enable(enable, safety);
}
EXPORT_SYMBOL_GPL(qpnp_torch_control_enable);

int qpnp_chg_torch_is_enable(void)
{
	int status = 0;

	if (the_chip)
	{
		if (the_chip->torch_control_status)
		{
			status = the_chip->torch_control_status;
		}
		else
		if (the_chip->torch_control_detect)
		{
			status = QPNP_TORCH_CONTROL_DETECT;
		}
		the_chip->torch_control_detect = false;
	}
...
EXPORT_SYMBOL_GPL(qpnp_chg_torch_is_enable);
...
qpnp_charger_probe(struct spmi_device *spmi)
{
	...
	mutex_init(&chip->torch_control_lock);
#endif /* CONFIG_BATTERY_SH */
...
#ifdef SHCHG_ENABLE_EXTERNAL_OVP_FET
	/* store iusb_trim data */
	qpnp_chg_charger_iusb_trim_store();
#endif /* SHCHG_ENABLE_EXTERNAL_OVP_FET */
	...
	INIT_DELAYED_WORK(&chip->torch_check_work, qpnp_torch_check_work);
#ifdef SHCHG_TORCH_FAILSAFE
	INIT_DELAYED_WORK(&chip->torch_safety_work, qpnp_torch_safety_work);
#endif /* SHCHG_TORCH_FAILSAFE */
#endif /* CONFIG_BATTERY_SH */
...
qpnp_charger_remove(struct spmi_device *spmi)
{
	...
	cancel_delayed_work_sync(&chip->torch_check_work);
#ifdef SHCHG_TORCH_FAILSAFE
	cancel_delayed_work_sync(&chip->torch_safety_work);
#endif /* SHCHG_TORCH_FAILSAFE */
	mutex_destroy(&chip->torch_control_lock);
#endif /* CONFIG_BATTERY_SH */
