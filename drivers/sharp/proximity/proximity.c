...
#define CALI_BUFFER_SIZE		12
#define CALI_MEASURE_DATA_COUNT 10
#define CALI_THRESHOLD_FILE_PATH ("/durable/sensor/proximity/prox_threshold.dat")
...
static int    proximity_calib_high_const = 0x4e;
static int    proximity_calib_low_const = 0x2e;
    ...
    module_param(proximity_calib_high_const, int, 0600);
    module_param(proximity_calib_low_const, int, 0600);
...
static ssize_t prox_kernel_read(struct file *fp, char *buf, size_t size , unsigned int offset);
static ssize_t prox_kernel_write(struct file *fp, const char *buf, size_t size);
int PROX_read_cali_ths(struct shdisp_prox_params *prox_params);
...
static int IOECS_Calibration(void);
...
static unsigned int calib_data_count = 0;
static unsigned int tmp_threshold = -1;
static atomic_t update_threshold = ATOMIC_INIT(0);
static atomic_t calibration_mode = ATOMIC_INIT(0);
...
int PROX_read_cali_ths(struct shdisp_prox_params *prox_params)
{
    char buf[CALI_BUFFER_SIZE], *sptr, *tmp_proxadj;
    struct file *fp;
    ssize_t size;
    int i;
    ...
    fp = filp_open(CALI_THRESHOLD_FILE_PATH, O_RDONLY, 0);
    if (IS_ERR_OR_NULL(fp)) {
        DEBUG_ERROR_LOG("Cannot open file: %s err=%d, pid=%d tgid=%d comm=%s\n", CALI_THRESHOLD_FILE_PATH, (int)fp, current->pid, current->tgid, current->comm);
        return SH_PROXIMITY_RESULT_FAILURE;
    }

    size = prox_kernel_read(fp, buf, CALI_BUFFER_SIZE, 0);
    if (size < CALI_BUFFER_SIZE) {
        DEBUG_ERROR_LOG("Cannot read file: %s err=%d, pid=%d tgid=%d comm=%s\n", CALI_THRESHOLD_FILE_PATH, (int)size, current->pid, current->tgid, current->comm);
        filp_close(fp, NULL);
        return SH_PROXIMITY_RESULT_FAILURE;
    }
    sptr = buf;
    ...
    for(i=0; i < 2; i++) {
        tmp_proxadj = strsep(&sptr, " ");
        if (tmp_proxadj != NULL) {
        proxadj[i] = atoi(tmp_proxadj);
        }
    }
    filp_close(fp, NULL);
...
static int IOECS_Enable(void)
{
    ...
    if(atomic_read(&calibration_mode) == 0) {
    ...
    if(atomic_read(&calibration_mode) == 0) {
        if(atomic_read(&enable_mode) == 0) {
            ...
            if (PROX_read_cali_ths(&prox_params) != SH_PROXIMITY_RESULT_SUCCESS) {
                memset((void*)proxadj, 0x00, sizeof(proxadj));
                p_sh_smem_common_type = sh_smem_get_common_address();
...
static int IOECS_Calibration(void)
{
    ...
    prox_unsubscribe();
    schedule_delayed_work(&calib_work, msecs_to_jiffies(500));
...
static int PROX_release(struct inode *inode, struct file *filp)
{
    ...
    if(atomic_read(&calibration_mode) == 1) {
        cancel_delayed_work_sync(&calib_work);
        atomic_set(&calibration_mode, 0);
        DEBUG_SENSOR_LOG("calib_work is canceled");
	}
                         ...
                         unsigned int cmd, unsigned long arg)
{
    ...
    case ECS_IOCTL_HT_THRESHOLD_WRITE:
        ...
        break;
    case ECS_IOCTL_CALIBRATION:
        DEBUG_LOG("ECS_IOCTL_CALIBRATION");
        if(IOECS_Calibration() < 0) {
            DEBUG_ERROR_LOG("ECS_IOCTL_CALIBRATION ERR");
            return -EIO;
        }
        break;
...
static void PROX_calib_workfunc(struct work_struct *work)
{
    int nResult = SH_PROXIMITY_RESULT_FAILURE;
    unsigned char lData  = 0x00;
    unsigned short LData = 0x00;
    unsigned char mData  = 0x00;
    unsigned short MData = 0x00;
    unsigned int psresult;

    struct file *fp; 
    char buf[CALI_BUFFER_SIZE];

    FUNC_LOG();

    if (calib_data_count < CALI_MEASURE_DATA_COUNT) {
...
//        if(atomic_read(&enable_mode) == 1) {
        ...
        psresult = (( MData<<8 ) | LData );
        tmp_threshold = (tmp_threshold < psresult) ? (tmp_threshold) : (psresult);
        DEBUG_SENSOR_LOG("psresult = 0x%04x, tmp_threshold = 0x%04x", psresult, tmp_threshold);
//        }
        calib_data_count ++;
    ...
    if (calib_data_count == CALI_MEASURE_DATA_COUNT) {
        ...
        Threshold_Low = tmp_threshold + proximity_calib_low_const;
        Threshold_High = tmp_threshold + proximity_calib_high_const;
        atomic_set(&update_threshold, 1);
...
//            return nResult;
        ...
        fp = filp_open(CALI_THRESHOLD_FILE_PATH, O_WRONLY | O_CREAT | O_TRUNC, 0664);
        if (IS_ERR_OR_NULL(fp)) {
            DEBUG_ERROR_LOG("Cannot create file: %s err=%d\n", CALI_THRESHOLD_FILE_PATH, (int)fp);
        } else {
            sprintf(buf, "%d %d", Threshold_Low, Threshold_High);
            prox_kernel_write(fp, buf, CALI_BUFFER_SIZE);
            filp_close(fp, NULL);
        }
...
proc_fs_error:
    ...
    INIT_DELAYED_WORK(&calib_work, PROX_calib_workfunc);
