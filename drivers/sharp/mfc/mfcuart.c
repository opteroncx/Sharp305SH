...
/* Common UART */
#define D_UART_DEV_FILE_PATH	("/dev/ttyHSL2")
...
static ssize_t uart_read(struct file *filp, char __user *buf, size_t len, loff_t *ppos)
{
	...
	if (tty && tty->f_op->read) {
		...
		if (result > 0) {
			char tmp[512];
			int tmp_len = (result > 512) ? 512 : result;
			if (copy_from_user(tmp, buf, tmp_len) == 0) {
				char line[16 * 5 + 1];
				int j, remain = tmp_len;
				char *p = tmp, *q;
				while (remain > 0) {
					q = line;
					for (j = 0; (j < 16) && (remain > 0); j++) {
						sprintf(q, " %02X", *p);
						p++;
						q += 3;
						remain--;
					}
					MFC_DRV_DBG_LOG("%s", line);
				}
			}
		}
#endif /* DEBUG_MFC_DRV */
...
static ssize_t uart_write(struct file *filp, const char __user *data, size_t len, loff_t *ppos)
{
	...
	if (tty && tty->f_op->write) {
		...
		if (len > 0) {
			char tmp[512];
			int tmp_len = (len > 512) ? 512 : len;
			if (copy_from_user(tmp, data, tmp_len) == 0) {
				char line[16 * 5 + 1];
				int j, remain = tmp_len;
				char *p = tmp, *q;
				while (remain > 0) {
...
static int uart_open(struct inode *inode, struct file *filp)
{
	...
	if (tty->f_op->unlocked_ioctl) {
		...
		/* set UART speed */
			result = tty->f_op->unlocked_ioctl(tty, TCGETS, (unsigned long)&termios);
			if (result < 0) {
				MFC_DRV_ERR_LOG(" get UART speed  = %d", result);
				filp_close(tty, 0);
				filp->private_data = NULL;
				result = -ENOSYS;
				return result;
		...
		termios.c_cc[VTIME] = 0;
			...
			result = tty->f_op->unlocked_ioctl(tty, TCSETS, (unsigned long)&termios);
			if (result < 0) {
				MFC_DRV_ERR_LOG(" set UART speed  = %d", result);
				filp_close(tty, 0);
				filp->private_data = NULL;
				result = -ENOSYS;
				return result;
