...
/* Define for disconnected ADC channel */
#if 1
#define SHBATT_DRV_DISCONNECT_ADC_CHANNEL
#endif
...
/* msm-gpio */
#define SHBATT_GPIO_TPIN_STATUS		56
...
static shbatt_result_t shbatt_control_read_adc_channel( shbatt_adc_t* adc_p, uint32_t* microvolts )
{
	static const shbatt_drv_adc_read_t adc_read[NUM_SHBATT_ADC_CHANNEL] =
	{
		...
		{	SHBATT_DRV_ADC_TYPE_VOLTAGE,	SHBATT_DRV_ADC_CHANNEL_MSM_THERM,	},	/* MPP_06 */
...
#ifdef SHBATT_DRV_DISCONNECT_ADC_CHANNEL
	if(adc_p->channel == SHBATT_ADC_CHANNEL_LCD_THERM || adc_p->channel == SHBATT_ADC_CHANNEL_DCIN ||
	   adc_p->channel == SHBATT_ADC_CHANNEL_CAM_THERM || adc_p->channel == SHBATT_ADC_CHANNEL_PA_THERM1)
	{
		result = SHBATT_RESULT_FAIL;
		SHBATT_ERROR("specified channel does not exist. channel = %d\n", adc_p->channel);
		adc_p->adc_code		= 0;
		adc_p->measurement	= 0;
		adc_p->physical		= 0;
		*microvolts			= 0;
...
#endif /* SHBATT_DRV_DISCONNECT_ADC_CHANNEL */
...
static int shbatt_drv_probe( struct platform_device* dev_p )
{
	...
	limit_lock_init(&shbatt_limit_lock[1],  PERF_LOCK_384000KHz,  "shbatt_lowbatt_limit_1");
	limit_lock_init(&shbatt_limit_lock[2],  PERF_LOCK_600000KHz,  "shbatt_lowbatt_limit_2");
	limit_lock_init(&shbatt_limit_lock[3],  PERF_LOCK_787200KHz,  "shbatt_lowbatt_limit_3");
	limit_lock_init(&shbatt_limit_lock[4],  PERF_LOCK_998400KHz,  "shbatt_lowbatt_limit_4");
	limit_lock_init(&shbatt_limit_lock[5],  PERF_LOCK_1094400KHz, "shbatt_lowbatt_limit_5");
	limit_lock_init(&shbatt_limit_lock[6],  PERF_LOCK_1190400KHz, "shbatt_lowbatt_limit_6");
#endif /* SHBATT_ENABLE_LIMIT_LOCK */
...
shbatt_result_t shbatt_api_notify_power_supply_changed( shbatt_ps_category_t cat )
{
	shbatt_result_t result = SHBATT_RESULT_SUCCESS;
...
#endif
			if(qpnp_chg_torch_is_enable() != 0)
			{
				const char *status = status_text[SHBATT_CHG_STATUS_DISCHARGING];
				SHBATT_TRACE("/battery/status = %s\n",status);
				size = snprintf(buf_p, PAGE_SIZE, "%s\n",status);
				break;
			}
